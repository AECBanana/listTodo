...
function collectChildIds(parentId: string, nodes: ProjectNode[]): Set<string> {
...
async function updateProject(id: string, req: UpdateProjectRequest) {
  ...
  if (!existing) return;
  const updated = { ...existing, ...req, updated_at: new Date().toISOString() };
...
async function createTag(req: CreateTagRequest) {
  const id = crypto.randomUUID();
  const tag: Tag = { id, name: req.name, color: req.color || "#808080" } as Tag;
